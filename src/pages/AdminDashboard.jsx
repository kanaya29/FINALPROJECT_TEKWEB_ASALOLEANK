...
const AdminDashboard = ({ events }) => {
  ...
  const addEvent = async (formData) => {
    try {
      const newTicket = {
        ...
        soldTicket: Number(formData.soldTicket) || 0,
      };

      await addDoc(collection(db, "tickets"), newTicket);
      showNotification("Event baru berhasil dipublikasikan!");
    } catch (error) {
  ...
  const updateEvent = async (formData) => {
    try {
      ...
      showNotification("Data event berhasil diperbarui!");
    } catch (error) {
      console.error("Error: ", error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f7ff] relative"> 
      ...
      <div className="max-w-7xl mx-auto py-10 space-y-10 px-4">
        <div className="bg-blue-500 rounded-3xl p-8 text-white shadow-xl flex justify-between items-center overflow-hidden relative">
